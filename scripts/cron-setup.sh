#!/usr/bin/env bash
# 安装 BYDFi SEO Audit 的 cron 任务（daily-pull + watch + quarterly review）
...
# 移除旧 BYDFi SEO 任务
sed -i.bak '/# BYDFi-SEO-Audit/d' "$CRON_FILE"

# 添加新任务
cat >> "$CRON_FILE" << EOF

# BYDFi-SEO-Audit Daily Rule Sync (每日 06:00 UTC)
...
crontab -l | grep BYDFi-SEO-Audit
echo ""
