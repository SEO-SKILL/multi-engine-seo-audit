# BYDFi Google 人工处置案例库

> 创建：2026-06-10
> 维护：kelly@bydfi.com
> 用途：积累每次 BYDFi 被 Google 处置或近处置的案例，反向沉淀为规则
> 引用方式：每条案例对应 `rules/bydfi/` 下的具体规则
...
|---|---|---|---|---|
| BYDFI-2026Q1-001 | 2026-Q1 | 已被处置 | 转载 + ticker 误识别 + schema 错配 | bydfi.l02.* / bydfi.l03.* / bydfi.l04.* |

---

## BYDFI-2026Q1-001：MEXC 页面被 Google 人工处置

### 一句话描述

BYDFi 转载 BlockchainReporter 文章发到 `/news/{id}`，因「转载缺原创增量 + ticker 误关联 + schema 内容错配 + hreflang 冲突」综合触发 Google 人工处置。

### 时间线

- T-0：BlockchainReporter 发布原文
- T+N：BYDFi 转载到 `/en/news/1057200`，移除作者 bio / FAQ / 部分外链
...
**Issue**：MEXC 页主体几乎是 BlockchainReporter 原文转载，只保留来源链接，但移除了原文的人类作者 Max Clark、作者 bio、FAQ、目录和部分外链来源。

**Why**：Google 对金融/科技/收益相关内容更看重 E-E-A-T、来源、责任归属和独特价值。单纯转载且 self-canonical，容易被判为低增量内容。

**Fix**：二选一：
- 要么 `noindex` 或 cross-domain canonical 到原文
- 要么补 MEXC 自有编辑评审、摘要、风险边界、事实核验、原作者信息和独立增量段落

**对应规则**：`bydfi.l01.republished-content-low-increment`（待写）
...
**对应规则**：`bydfi.l02.ticker-context-mismatch` ✅（已写 in `pros-ticker-blacklist.yaml`）
...
**Issue**：渲染后有 NewsArticle JSON-LD，但 raw HTML 中没有；schema 里还塞了 MEXC App 的 AggregateRating，页面可见内容并不是 MEXC App 评价。copyrightNotice 邮箱也和页面可见邮箱不一致。
...
**对应规则**：`bydfi.tagging.topic-mismatch`（待写）
...
> 持续收集 MEXC / Bybit / KuCoin / Crypto.com 等同行公开的 Google 处置案例
