...
| 本地缓存 | `diskcache 5.6.3` | 无需 Redis，适合单租户 Platform 和本地抓取缓存；PRD 只要求 V1 单租户（PRD §4、SKILL 项目范围）。PyPI：https://pypi.org/project/diskcache/ | SQLite 自写；Redis 适合 V2 多租户/并发 |
...
- `id`：稳定规则 ID，如 `platform.L02.ticker-context-mismatch`。
- `source`：`hard_rule|llm|external_api|diff`。
- `platform`：`google|bing|naver|yandex|llm-engines|shared|platform`。
...
1. 初始化 `run_id/trace_id`，加载 `config.yaml` 项目配置和 platform routing（config `projects.platform.platform_routing`）。
...
5. 聚合时按 severity、confidence、平台权重、Platform 专属规则优先级计算 Final Verdict。
...
- PRD 指定单租户 Platform，multi-tenant 只保留 hook（PRD §4、SKILL 项目范围）。
...
- PRD V1 验收重点是 Platform 自有页面 audit/gate，不是大规模竞品爬取（PRD §2.1、§9.1）。
...
- 固定 system prompt：Platform SEO 审核口径、输出 schema、severity 定义。
- Platform 专属规则摘要：7 类事故、ticker 白名单/黑名单、E-E-A-T 风控口径（lessons L01-L06、PRD §1.1）。
...
- 影响 Platform 9 语言路由或金融/crypto 合规口径。
...
config 已明确 Platform locales 与 platform routing：
...
- 并行：technical-agent 同时应用 shared + google/bing/naver hard rules；semantic-agent 应用 Platform + Naver Korean content + Google helpful content；geo-agent 应用 LLM rules。
- 串行聚合：report-agent 合并平台 findings，若 Naver 本地化低质但 Google 无 blocker，Final Verdict 可为“改后再审”而不是“暂不上线”；若 Google/Platform 命中 blocker，则 gate 阻断。
...
- `snapshots/platform/audit/2026/06/10/<run_id>/`
- `snapshots/platform/gate/2026/06/10/<run_id>/`
- `snapshots/platform/compare/2026/06/10/<run_id>/`
- `snapshots/platform/watch/2026-W24/<run_id>/`
...
- Platform 自有页面 audit/gate：10 分钟，gate 可强制 bypass。
- competitor 页面：24 小时。
...
1. `case-exchange-incident.html`：覆盖 lessons L01-L06 和 PRD §1.1 七类事故。
...
- lessons L01-L06 + PRD §1.1 7 类问题做成 `test_case-exchange_incident_detects_all_required_findings`。
...
- 能力 #12 Log File SEO + Index Coverage：V1 先接 GSC/Cloudflare 基础字段；fly.io log 若 Platform 不实际使用则标 optional。
- 能力 #11 自进化：V1 先记录 feedback JSON，不自动改 rules；自动 rules 更新需人工 PR。

不能降级项：

- 某加密交易所行业案例 7 类问题 100% 检出（PRD §9.1）。
...
按 PRD §8.1，W1 是“框架骨架 + 5 个核心 sub-agent + Platform 规则库初始化”，负责方式是 Codex 写 / Claude 审。考虑 Phase 0 仍需 Claude 审报告，W1 建议如下。
...
| Day 4 | technical-agent MVP：schema/hreflang/canonical/robots/sitemap/url health 硬规则；某加密交易所案例 L03/L04 先覆盖 | Codex 写，Claude 审 | hard rules + unit tests 初版 |
...
- `case-exchange-incident` 离线 fixture 能检出 L01-L06 对应 finding。
