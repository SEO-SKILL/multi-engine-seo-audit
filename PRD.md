...
为 Platform 量身打造的「**SEO 风控 + 增长决策中台**」，覆盖 33 项业务能力 / 11 个框架模块 / 8 个搜索引擎平台规则库，目标是**永远不再发生 Google 人工处置**，同时跑通合约工具页 SEO MVP 增长闭环。
...
- **沉淀**：建立 Platform 专属 SEO 规则库 = 6 个月不可复制的护城河
...
Fetch / Parse / Analyze Layer
           │
           ▼
Rules Layer（platform/ + platforms/ + shared/）
           │
           ▼
...
| 周次 | 任务 | 负责 |
|---|---|---|
| W1 | 框架骨架 + 5 个核心 sub-agent + Platform 规则库初始化 | Codex 写 / Claude 审 |
| W2 | 5 个剩余 sub-agent + 5 个平台规则库 + 规则同步管道 | Codex 写 / Claude 审 |
| W3 | 4 个核心工作流串联 + golden fixture + 测试 | Codex 写 / Claude 审 |
| W4 | 文档 + Slack/Git 集成 + Platform 真实页面试跑 + 修 bug | Claude 主导 |
...
- **M2（W5 末）**：Platform 真实页面跑过 50+ 次审核，规则库初步沉淀
- **M3（W8 末）**：V2 核心能力上线
...
```
rules/
├── platform/                    # Platform 专属（核心护城河）
├── platforms/<engine>/
│   ├── _knowledge/           # 自研知识层（比 Will 深 Platform 对齐）
...
- 参考结构（5 字段 / 76 主题 / Platform 业务对齐方式）
- 自己写更深的 _knowledge MD（加 Platform 业务对齐 + GEO + AI Overview）
